...
//				swap(leftPtr, rightPtr);
	...
	private void reQuickSort(int left, int right) {
		if (right - left <= 0) {
			...
			String pivot = array[right].getIsbn();
...
//	}
	...
	public BookBag<Book> shallowCopy(){
		BookBag<Book> copy = new BookBag<Book>(maxSize);
		for(int i = 0; i < nElems; i++) {
			copy.insertT(array[i]);
		}
		return copy;
	}
	public BookBag<Book> reverseCopy(){
		BookBag<Book> copy = new BookBag<Book>(maxSize);
		for(int i = nElems-1; i > -1; i--) {
			copy.insertT(array[i]);
		}
